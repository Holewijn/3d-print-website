...
export default function QuotePage() {
  ...
  // Fetch available combos on mount
  useEffect(() => {
  ...
  // Distinct materials from combos
  const materials = useMemo(() => {
    const seen = new Map<string, { id: string; name: string }>();
    for (const c of combos) {
      if (!seen.has(c.materialId)) seen.set(c.materialId, { id: c.materialId, name: c.materialName });
    }
    return Array.from(seen.values()).sort((a, b) => a.name.localeCompare(b.name));
  }, [combos]);

  // Colors filtered to the chosen material
  ...
  // When material changes, reset color
  useEffect(() => {
    if (!materialId) { setColorId(""); return; }
    if (colorId && !colorsForMaterial.some((c) => c.colorId === colorId)) {
      setColorId("");
    }
    // Auto-pick first available color if nothing selected
    if (!colorId && colorsForMaterial.length > 0) {
  ...
  async function calculate(e: React.FormEvent) {
    e.preventDefault();
    if (!upload || !materialId || !colorId) return;
    setBusy(true); setErr("");
    try {
      const q = await api("/quotes", {
        method: "POST",
        body: JSON.stringify({
          stlUploadId: upload.id, email, materialId, colorId,
  ...
  function submitForApproval() {
    // Quote already exists in PRICED status; nothing more to do
    setSubmitted(true);
  }
          ...
          <div className="form-card">
            <form onSubmit={calculate}>
              ...
              <div className="form-row">
                ...
                <div>
                  <label>Color</label>
                  <select value={colorId} onChange={(e) => setColorId(e.target.value)} required disabled={!materialId}>
                    <option value="">— Select —</option>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Anything we should know? Deadlines, special handling, post-processing requests, etc."
                />
              </div>

              <button className="btn btn-lg" disabled={busy || !upload || !materialId || !colorId}>
          ...
          <div>
            ...
            {quote && !submitted && (
              <div className="quote-result" style={{ marginTop: "1.5rem" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                  <button type="button" className="btn btn-lg" onClick={payNow} disabled={paying}>
                    {paying ? "Redirecting to checkout…" : "Pay Now & Print →"}
                  </button>
                  <button type="button" className="btn btn-outline" onClick={submitForApproval}>
                    Submit for Approval (no payment)
                  </button>
                  <p style={{ fontSize: "0.78rem", color: "var(--text-muted)", textAlign: "center", margin: "0.25rem 0 0" }}>
            ...
            {submitted && (
              <div className="quote-result" style={{ marginTop: "1.5rem", textAlign: "center" }}>
                ...
                <h3>Submitted!</h3>
                <p style={{ color: "var(--text-muted)" }}>We've received your quote request and will get back to you within 24 hours at <strong>{quote.email}</strong>.</p>
              </div>
            )}
          </div>
        </div>
