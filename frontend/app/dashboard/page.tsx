        ...
        <div className="container">
          ...
          {orders.length === 0 ? <p style={{ color: "var(--text-muted)", marginBottom: "3rem" }}>No orders yet.</p> : (
            <div style={{ marginBottom: "3rem" }}>
              {orders.map(o => (
                <div key={o.id} className="card" style={{ marginBottom: "1rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontWeight: 700 }}>Order #{o.id.slice(-8)}</div>
                    <div style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>{new Date(o.createdAt).toLocaleDateString()} · {o.status}</div>
                  </div>
